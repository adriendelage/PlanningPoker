...
export default function GanttHome() {
  ...
  const create = () => {
    ...
    if (!name.trim()) return alert("Donne un nom au projet");
    socket.emit("gantt:create", { name, hostName }, (id) => {
      addLocalSession({ id, tool: "gantt", name, role: "host" });
      nav("/gantt/" + id);
    });
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0d0d1a", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
      <div style={{ width: "100%", maxWidth: 540, color: "#fff" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 32, fontWeight: 700, color: "#e63946" }}>Rétro-planning</h1>
          <p style={{ margin: "8px 0 0", color: "#555", fontSize: 15 }}>
            Tâches, dépendances et chemin critique — calculé automatiquement
          </p>
        </div>
        ...
        <div style={{ background: "#111", borderRadius: 16, padding: "28px 24px", border: "1px solid #222" }}>
          ...
          <input placeholder="Ex: Migration vers le cloud" value={name}
            onChange={e => setName(e.target.value)}
            onKeyDown={e => e.key === "Enter" && create()}
            style={inputStyle} />

          <button onClick={create}
            style={{ width: "100%", padding: 16, background: "#e63946", border: "none", borderRadius: 10, fontWeight: "bold", cursor: "pointer", fontSize: 17, color: "#fff" }}>
            Créer le planning →
          </button>

          <p style={{ margin: "16px 0 0", fontSize: 13, color: "#555", textAlign: "center" }}>
            Ajoute tes tâches avec leur durée (en jours) et leurs dépendances :
            le chemin critique se recalcule automatiquement à chaque changement.
          </p>
        </div>
