...
export default function CapacityHome() {
  ...
  const create = () => {
    ...
    socket.emit("capacity:create", { name, hostName }, (id) => {
      addLocalSession({ id, tool: "capacity", name, role: "host" });
      nav("/capacity/" + id);
    });
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0d0d1a", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
      <div style={{ width: "100%", maxWidth: 540, color: "#fff" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 32, fontWeight: 700, color: "#06d6a0" }}>Planificateur de capacité</h1>
          <p style={{ margin: "8px 0 0", color: "#555", fontSize: 15 }}>
            Estime la capacité du prochain sprint selon la disponibilité réelle de l'équipe
          </p>
        </div>
        ...
        <div style={{ background: "#111", borderRadius: 16, padding: "28px 24px", border: "1px solid #222" }}>
          ...
          <button onClick={create}
            style={{ width: "100%", padding: 16, background: "#06d6a0", border: "none", borderRadius: 10, fontWeight: "bold", cursor: "pointer", fontSize: 17, color: "#0d0d1a" }}>
            Créer le tableau →
          </button>

          <p style={{ margin: "16px 0 0", fontSize: 13, color: "#555", textAlign: "center" }}>
            Comme le Kanban, ce tableau est permanent : garde le lien et
            planifie la capacité à chaque nouveau sprint.
          </p>
        </div>
