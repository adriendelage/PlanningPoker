...
export default function PulseHome() {
  ...
  const create = () => {
    ...
    if (!name.trim()) return alert("Donne un nom à l'équipe");
    socket.emit("pulse:create", { name, hostName }, (id) => {
      addLocalSession({ id, tool: "pulse", name, role: "host" });
      nav("/pulse/" + id);
    });
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0d0d1a", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
      <div style={{ width: "100%", maxWidth: 540, color: "#fff" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 32, fontWeight: 700, color: "#ff6b9d" }}>Pouls d'équipe</h1>
          <p style={{ margin: "8px 0 0", color: "#555", fontSize: 15 }}>
            Un check-in d'humeur en un clic, chaque jour, avec la tendance dans le temps
          </p>
        </div>
        ...
        <div style={{ background: "#111", borderRadius: 16, padding: "28px 24px", border: "1px solid #222" }}>
          ...
          <button onClick={create}
            style={{ width: "100%", padding: 16, background: "#ff6b9d", border: "none", borderRadius: 10, fontWeight: "bold", cursor: "pointer", fontSize: 17, color: "#0d0d1a" }}>
            Créer le tableau →
          </button>

          <p style={{ margin: "16px 0 0", fontSize: 13, color: "#555", textAlign: "center" }}>
            Comme le Kanban, ce tableau est permanent : garde le lien et
            fais ton check-in chaque jour.
          </p>
        </div>
